...
void Skip(FILE *fin); // Пропуск пробелов в файле чтения
...
void Sostoyania(FILE *fin, FILE *fout)
{
    ...
    int step = Pow(2, n);
    int **arr = NULL;
    int *arr_out = (int *)calloc(step * 2, sizeof(int));
    int **arr_check = NULL;
    if (Memory(&arr, 2, step) && arr_out && Memory(&arr_check, step, step))
    {
        int flag = 1;
...
// Пропуск пробелов в файле и перевод корретки на следующую строку
void Skip(FILE *fin)
{
    char tmp;
    while (fscanf(fin, "%c", &tmp) == 1)
    {
        if (tmp == ' ')
            continue;
        if (tmp == '\n' || tmp == '\r')
