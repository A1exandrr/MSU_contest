...
{
    loop_node = MM_HEAD;
    while (loop_node && (loop_node->right_neigh != NULL) && strcmp(loop_node->obj_name, name))
    {
...
int mm_destroy_object(const char *name)
{
    ...
    {
        l_tmp = tmplnk->links;
        while (l_tmp && l_tmp->next_link && strcmp(l_tmp->linked_obj->obj_name, name))
        {
            l_tmp = l_tmp->next_link;
        }
...
int mm_link(const char *object1_name, const char *object2_name)
{
    ...
    if (!check_obj_existance(object2_name) || check_link_existance(tmp->links, loop_node->obj_name))
    {
        return 0;
    }
    struct link_node *new_link = (struct link_node *)malloc(sizeof(struct link_node));
...
{ //+
    ...
    struct ObjNode **collector = (struct ObjNode **)malloc(buff_size * sizeof(ObjNode *));
...
void mm_collect_live_objects()
{
    ...
    while ((j < buff_size) && collector[j])
    {
        ...
        {
            int k = 0;
            while ((k < i) && strcmp(collector[k]->obj_name, l_tmp->linked_obj->obj_name))
            {
                k++;
            }
            if (k == i)
