...
int create_dir(const char *path)
{
    return chkDsk(FM) == 0 ? 0 : fileNodeMkObjValidated((char *)path, 1, 0);
}
int create_file(const char *path, int file_size)
{
    return chkDsk(FM) == 0 ? 0 : fileNodeMkObjValidated((char *)path, 0, file_size);
}
int change_dir(const char *path)
{
    return chkDsk(FM) == 0 ? 0 : fileNodeChDirGlobal((char *)path);
}
int remove(const char *path, int recursive)
{
    return chkDsk(FM) == 0 ? 0 : fileNodeRmMode((char *)path, recursive);
}
...
int fileNodeMoveFM(int mode, char *path)
{
    if (mode == 0 || strcmp(FM->absolute_path, CUR_DIR) != 0 || strlen(path) > 1)
    {
        return fileNodeGoToDirGlobal();
    }
    if ((strcmp(path, "..") == 0 && strlen(path) == 2) && FM->parent != NULL)
    {
        FM = FM->parent;
        return 1;
    }
    else if (strcmp(path, ".") == 0 && strlen(path) == 2)
        return 1;
    ...
    else if (strlen(path) > 0)
    {
        ...
        if (chkdIndx >= 0)
        {
            return fileNodeGoDown(chkdIndx);
        }
        else
            return 0;
    }
...
int fileNodeGoToDirGlobal()
{
    ...
    while ((token = strsep(&string, "/")) != NULL)
    {
        if (strcmp(token, "") == 0 || strlen(token) <= 0 || token[0] == '\0')
            continue;
...
int fileNodeMkObjValidated(char *path, int mode, int fileSize)
{
    ...
    while ((token = strsep(&string, "/")) != NULL)
    {
        ...
        if (chkdIndx == -1 && mode == 0 && (fileSize > DskSz || fileSize + OcpdSz > DskSz || fileSize < 0))
        {
...
int fileNodeMkObj(char *objName, int mode, int file_size)
{
    ...
    ++FM->heirsCount;
    if (FM->heirs == NULL)
        FM->heirs = (fileNode **)malloc(sizeof(fileNode *));
    else
    ...
    newFm->absolute_path = (char *)realloc(
        newFm->absolute_path, sizeof(char) * (strlen(newFm->absolute_path) + strlen(objName) + strlen("/\0")));
    ...
    OcpdSz += file_size;
    if (mode == 1)
        fprintf(stdout, "success: created directory %s in %s\n", FM->heirs[FM->heirsCount - 1]->name,
                FM->absolute_path);
    if (mode == 0)
        fprintf(stdout, "success: created file %s with size %lld bytes\nFree space: %d bytes\n",
...
int create(int disk_size)
{
    if (chkDsk(FM))
    {
        fprintf(stdout, "error: file manager already exists\n");
        return 0;
    }
    else if (disk_size <= 0)
    {
        fprintf(stdout, "error: wrong disk size\n");
        return 0;
    }
    else
    {
...
int fileNodeGoDown(int heirIndx)
{
    if (heirIndx == -1 || FM->heirs[heirIndx]->isDir == 0)
        return 0;
